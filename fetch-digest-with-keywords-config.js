...
async function searchNewsForTopic(topic) {
    ...
    for (let i = 0; i < topic.keywords.length; i++) {
        ...
        // 不限制網域,使用精確的關鍵字組合來過濾結果
        const url = `https://newsapi.org/v2/everything?q=${encodeURIComponent(keyword)}&language=zh&sortBy=publishedAt&pageSize=5&apiKey=${NEW_API_TOKEN}`;
